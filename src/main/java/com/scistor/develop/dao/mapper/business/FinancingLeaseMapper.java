...
import com.scistor.develop.data.entity.FinancingLease;
...
@Mapper
public interface FinancingLeaseMapper {

    @Results(id = "FinancingLeaseResultMap", value = {
    ...
    @Select("select * from financing_lease where delete_flag=0 and ${wheres} order by ${orderCloumn} ${orderDesc}  limit #{start},#{end}")
    List<FinancingLease> listByConditions(@Param("wheres") String wheres,
    ...
    @Select("select ${caseSql} financing_lease.* from financing_lease where delete_flag=0 and ${wheres} order by ${orderCloumn} ${orderDesc}  limit #{start},#{end}")
    ...
    @Select("select count(1) from financing_lease where delete_flag=0 and ${wheres} ")
    long countByConditions(@Param("wheres") String wheres);
    @Select("select * from financing_lease where id=${id}")
    @ResultMap(value = "FinancingLeaseResultMap")
    FinancingLease getFinancingLeaseById(@Param("id") Long id);
}
